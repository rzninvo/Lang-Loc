...
def run_eval_table1(cfg):
    """Run Table 1 evaluation with Hydra config."""
    ...
    scanscribe_test_55 = torch.load(cfg.graphs_scanscribe_test, weights_only=False, map_location='cpu')
    query_graphs = {}
    ...
    scanscribe_218 = torch.load(cfg.graphs_scanscribe_pool, weights_only=False, map_location='cpu')
    pool_graphs = {}
    ...
    scanscribe_acc = eval_acc_dual_aligner(
        model,
        _3dssg_graphs,
        query_dataset=list(query_graphs.values()),
        pool_dataset=list(pool_graphs.values()),
