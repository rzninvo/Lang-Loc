...
from langloc.retrieval.scene_graph_utils import check_valid_graph
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Test scene graph loading')
    parser.add_argument('--graphs_3dssg', type=str, required=True, help='Path to 3DSSG graphs .pt file')
    parser.add_argument('--graphs_scanscribe', type=str, required=True, help='Path to ScanScribe graphs .pt file')
    parser.add_argument('--max_dist', type=float, default=1.0)
    parser.add_argument('--embedding_type', type=str, default='ada')
    args = parser.parse_args()

    ######## 3DSSG #########
    _3dssg_scenes = torch.load(args.graphs_3dssg, weights_only=False)
    ...
    ######### ScanScribe #########
    scanscribe_scenes = torch.load(args.graphs_scanscribe, weights_only=False)
