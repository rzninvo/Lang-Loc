...
def filter_candidates_by_observed(
    candidates: Dict[str, Set[str]],
    observed_present: Set[str],
    observed_absent: Set[str],
) -> Dict[str, Set[str]]:
    """Keep views that contain ALL present items, and **do not** contain any 'absent' items (strict)."""
    pruned = {}
    for vname, vitems in candidates.items():
        if observed_present.issubset(vitems) and not (vitems & observed_absent):
            pruned[vname] = vitems
    return pruned
...
def rarest_item(candidates: Dict[str, Set[str]],
    ...
    """
    Choose the item that appears in the fewest remaining candidate views (but at least 1).
    Break ties randomly. Returns (item, freq).
    If no item is available (empty candidates), return ("", 0)
    """
    if not candidates:
        return ("", 0)

    # Count frequency across candidate views
    freq: Dict[str, int] = {}
    ...
    # Find the minimum positive frequency
    min_freq = min(freq.values())
    rare_items = [it for it, c in freq.items() if c == min_freq]
    choice = random.choice(rare_items)
...
def main():
    ...
    while True:
        ...
        # Re-filter candidates with new info
        new_candidates = filter_candidates_by_observed(candidates, observed_present, observed_absent)

        # If we eliminated all candidates by mistake, roll back the last 'absent' constraint once
