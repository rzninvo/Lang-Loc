...
echo "[INFO] Step 1/2: Downloading and extracting data for $SCAN_ID (dataset=$DATASET)..."
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
"$SCRIPT_DIR/download_subset.sh" --dataset "$DATASET" "$SCAN_ID"

# -------- RUN KEYFRAME GENERATION --------
echo "[INFO] Step 2/2: Running keyframe generation for $SCAN_ID (dataset=$DATASET)..."
